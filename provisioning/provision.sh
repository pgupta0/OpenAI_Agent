...
#   2. Paste their provider API key when prompted by student_start.sh
#      (OpenAI by default in this fork; see dtlab_config.env)
...
# Provider SDK install is conditional on DTLAB_PROVIDER (dtlab_config.env)
# -- read directly from the kit's own copy, since this runs before that
# file is copied to ~/dtlab/. Default matches the runtime fallback used
# everywhere else in this kit (student_start.sh: ${DTLAB_PROVIDER:-anthropic}).
...
if [ "$KIT_PROVIDER" = "openai" ]; then
  ...
  # unlike Anthropic's native anthropic_messages adapter, it needs no
  # extra SDK pulled into the Hermes venv. Nothing to install here.
  echo "DTLAB_PROVIDER=openai (dtlab_config.env) -- Hermes' built-in OpenAI"
  echo "provider needs no extra SDK install; skipping."
else
